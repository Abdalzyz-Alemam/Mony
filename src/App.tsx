...
import { LogOut, LayoutDashboard, Receipt, CheckSquare, Settings, Languages } from 'lucide-react';
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#f5f5f5] p-6 text-center">
        ...
        >
          <div className="space-y-2">
            <h1 className="text-4xl font-light tracking-tight text-slate-900">{t('appName')}</h1>
            <p className="text-slate-500">{t('appTagline')}</p>
          </div>
          <Button onClick={login} className="w-full bg-slate-900 text-white hover:bg-slate-800 h-12 rounded-xl text-lg shadow-lg shadow-slate-200">
            {t('signIn')}
          </Button>
          
          <div className="pt-4">
            <Button variant="ghost" size="sm" onClick={() => setLanguage(language === 'ar' ? 'en' : 'ar')} className="text-slate-400 hover:text-slate-900">
